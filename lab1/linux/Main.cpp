...
void testVectorLibraryDynamic() {
    ...
    void* hDLL = dlopen("./libDynamicLib.so", RTLD_LAZY);
    if (!hDLL) {
    ...
    AddVectorsFunc AddVectors = (AddVectorsFunc)dlsym(hDLL, "AddVectors");
    GetLengthFunc CalculateVectorLength = (GetLengthFunc)dlsym(hDLL, "CalculateVectorLength");
